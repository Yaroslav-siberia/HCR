...
def scaner(image):
    ...
    #https://robocraft.ru/blog/computervision/484.html
    ...
    allContours = cv2.findContours(edgedImage.copy(), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    allContours = imutils.grab_contours(allContours)  # RETR_LIST
    ...
    try:
        # ищем самый большой контур, потенциально это лист тетрадный, можем его не найти
        ...
        print(f'Не найдено контуров')
        # если произошла ошибка - возвращаем оригинал
        return False, orig
    try:
        # вырезаем самый большой контур(лист) и делаем его прямоугольным. если контур имеет больше 4 вершин то ошибка будет
        ...
        # все отработало, мы вырезали и отсканировали изображение
        return True, scanBW
