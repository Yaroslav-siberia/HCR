...
class Model:
    ...
    def to_sparse(self, texts: List[str]) -> Tuple[List[List[int]], List[int], List[int]]:
        """кладем истинный текст на картинке в тензор для ctc_loss."""
        indices = []
        values = []
    ...
    def train_batch(self, batch: Batch) -> float:
        """передача следующего батча на НС для обучения"""
    ...
    def dump_nn_output(rnn_output: np.ndarray) -> None:
        """дамп нейронки для промежуточного сохранения"""
        dump_dir = '../dump/'
    ...
    def infer_batch(self, batch: Batch, calc_probability: bool = False, probability_of_gt: bool = False):
        """передача батча на НС для рапознавания"""
