...
# dataclasses in python. решил попробовать)
...
def detect(img: np.ndarray,
    ...
    '''
    метод пространственной сегментации слов от R. Manmatha.
    http://ciir.cs.umass.edu/pubfiles/mm-27.pdf.

    Детектирует слова. Метод частично связан с бинаризацией. шрубо говоря такая последовательности
    Бинаризируем картинку
    ищем контуры слов.
    :param img: изображение grayscale uint8
    :param kernel_size: Размер ядра , нечетное целое
    ...
    :param min_area: мин площадь слова, если меньше то игнорируем
    :return: список детектов, кажй детект - рамка вокруг слова
    '''
...
def prepare_img(img: np.ndarray,
    ...
    '''
    конвертирует изображение в grayscale если надо и меняет масштаб
    :param img:
    :param height:
    :return:
    '''
